...
func (r *UserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if _, ok := user.Labels["iam.kubesphere.io/origin-uid"]; !ok {
		err := fmt.Errorf("非登录用户: \"%v\" ，没有电话号码标签", uName)
	...
	//删除在4A中失效的账号
	active, userName := CheckStatus(r.AppId, r.AppSecret, r.Api, uTel)
	switch {
	case !active:
		log.V(1).Info("4A账号失效", "username", uName, "tel", uTel, "desc", user.Annotations["kubesphere.io/description"])
		if err := r.Delete(ctx, user); err != nil {
			return ctrl.Result{}, err
		}
	case active && userName != "":
		if _, ok := user.Labels["app.auth.cmft"]; !ok {
			user.Labels["app.auth.cmft"] = userName
		}
	}
